...
- **NEW:** Unified config system (`~/.adw/config.toml`) with sections
- **NEW:** Config CLI: show, get, set, keys, edit, reset, migrate
...
- [x] **P11-4** Config consolidation ✅
  ...
  - Single config file: `~/.adw/config.toml` with sections:
    - `[core]` - tasks_file, default_workflow, default_model
    - `[daemon]` - poll_interval, max_concurrent, auto_start
    - `[ui]` - show_logo, theme, log_level, notification sounds
    - `[workflow]` - timeout, retries, checkpoints, wip commits
    - `[workspace]` - worktrees, default_branch, active_workspace
    - `[slack]`, `[linear]`, `[notion]`, `[github]`, `[webhook]` - integrations
    - `[plugins]` - plugin-specific settings
  - Environment variable overrides for all settings
  - Clear defaults for all options
  - CLI commands:
    - `adw config show [--secrets] [--json] [--section]`
    - `adw config get <key>` - Get value by dotted path (e.g., core.default_model)
    - `adw config set <key> <value>` - Set and save config value
    - `adw config keys [--section]` - List all available keys
    - `adw config edit` - Open in $EDITOR
    - `adw config path` - Show config file path
    - `adw config reset [--section] [--yes]` - Reset to defaults with backup
    - `adw config migrate` - Migrate from workspace.toml
