...
**Current Phase:** 1 (Observability & Safety - Hooks Integration Complete)
**Status:** Phase 1 hooks integration complete. Ready for screenshot capture and event dashboard.
...
| 8 - Failure Recovery | Not Started | 0% |
...
**Status:** 20% Complete (basic retry exists)

### What Exists
- Basic retry loop in `src/adw/agent/executor.py` (`prompt_with_retry()`)
- `RetryCode` enum in `src/adw/agent/models.py`
- No test framework detection or test execution

### Test Validation Tasks

- [ ] **P2-1** Create `src/adw/testing/detector.py`
  - Detect: pytest, jest, vitest, npm test
  - Check config files: pytest.ini, package.json, vitest.config.ts
  - Return framework name and test command

- [ ] **P2-2** Create `src/adw/testing/runner.py`
  - `run_tests(command: str) -> TestResult`
  - Run via subprocess with 5-minute timeout
  - Capture stdout/stderr, parse exit codes
  - Stream output for long-running suites

- [ ] **P2-3** Create `TestResult` dataclass
  - Fields: passed, failed, skipped counts
  - Failed test names and error messages
  - Coverage percentage (optional)
  - Support pytest, jest, vitest output formats

- [ ] **P2-4** Integrate test execution into SDLC workflow
  - Run tests after implement phase
  - Feed test errors back to agent context
  - Re-run implementation (max 3 retries)
  - Add `--no-tests` flag to skip validation

- [ ] **P2-5** Add test results to task completion
  - Format: "Tests: X passed, Y failed, Z skipped"
  - List failed test names if any

### Smart Retry Tasks

- [ ] **P2-6** Create `src/adw/retry/context.py`
  - `build_retry_context(error: str, phase: str) -> str`
  - Format errors with phase, message, instruction
  - Truncate stack traces to prevent token overflow

- [ ] **P2-7** Add retry configuration
  - Config options: `max_retries` (default 3), `retry_delay_seconds` (default 2)
  - Per-task override via task config
  - Exponential backoff: 2s, 4s, 8s

- [ ] **P2-8** Implement progressive retry strategies
...
- [ ] **P2-9** Implement escalation protocol
  - After max retries, escalate to human
  - Generate `agents/<task_id>/escalation.md` with:
    - Task description, all error messages
    - Attempted solutions, suggested next steps
  - Send notification if configured

- [ ] **P2-10** Add retry logging
  - Log each attempt: number, triggering error, strategy used, duration
...
- [ ] **P8-1** Create `src/adw/recovery/classifier.py`
...
- [ ] **P8-2** Implement recovery strategies
  - `RetryStrategy` - Simple retry with exponential backoff
  - `FixStrategy` - Auto-fix based on error type
  - `SimplifyStrategy` - Reduce task scope
  - `EscalateStrategy` - Escalate to human

- [ ] **P8-3** Add partial commit system
  - Commit working changes with `[WIP]` prefix
  - Track completed vs remaining work
  - Enable resume from checkpoint

- [ ] **P8-4** Create `src/adw/recovery/checkpoints.py`
  - Save state after each successful step
  - Store in `agents/<task_id>/checkpoints/`
  - Support `adw resume <task_id>` CLI

- [ ] **P8-5** Add rollback capability
  - `adw rollback <task_id>` - Undo all task changes
  - Git revert commits, cleanup worktree
  - Return task to "failed" state

- [ ] **P8-6** Generate escalation reports
  - Human-readable failure report with:
    - Task description, error messages, stack traces
    - Attempted solutions, modified files
